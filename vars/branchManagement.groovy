    ...
    node {
        ...
        try {
            ...
            dir("${projectName}-${teamName}-${artifactId}") {
                ...
                switch (operation) {
                    case "release-create":
                        branchManagerService.createRelease(params.projectName, params.teamName, params.artifactId)
                        break
                    case "release-close":
                        branchManagerService.finishRelease(params.projectName, params.teamName, params.artifactId)
                        break
                    case "release-candidate-create":
                        branchManagerService.createReleaseCandidate(params.projectName, params.teamName, params.artifactId)
                        break
                    case "feature-create":
...
def runCommand(String command) {
    if (!isUnix()) {
        ...
        if (command.trim().toLowerCase().startsWith("mvn")) {
            withMaven(globalMavenSettingsConfig: 'cicd_jfrog_final', maven: 'apigee-maven') {
